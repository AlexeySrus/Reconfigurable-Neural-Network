...
```
На данный момент в библеотеке есть заготовленная функцйия структуры сети `DEFAULT_TRAPEZE<unsigned int coeff>`, описывающая трапецию, где `coeff`\ --\ коэффициент, определяющий во сколько раз первый скрытый слой будет больше входного.
